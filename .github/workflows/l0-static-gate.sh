...
#        只查语法；以下六族类型错误仅 macOS L1 编译门禁可暴露，静态启发式左移拦截：
...
# ---------- [15] 类型层启发式门禁 ----------
section "15/16" "类型层启发式 —— 跨层 import 覆盖/Date·Double 混比/iOS 专用符号守卫/Linux 桩守卫外使用/any X? 拼写/nil→String 实参（六族 CI 实证左移）"
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if [ -z "$t_scanned" ]; then
    ...
    pass "$(printf '%s\n' "$THEUR" | sed -n 's/^__SCANNED__ //p') 个文件通过六族启发式"
  fi
fi
