...
struct RuleExtractorTests {
    ...
    /// 原名：有列头的三药表格每药一行八字段
    @Test func threeDrugTableWithColumnHeaderYieldsEightFieldsPerRow() {
    ...
    /// 原名：两行式处方用法行并入上一药品行不丢第二药
    @Test func usageLineMergesIntoPreviousDrugRowWithoutLosingSecondDrug() {
    ...
    /// 原名：混排整行五要素全拆
    @Test func mixedLineSplitsAllFiveElements() {
    ...
    /// 原名：无空格规格行锚不丢
    @Test func specWithoutSpaceStillKeepsRowAnchor() {
    ...
    /// 混排药名判真（须后拆分），纯药名判假。
    /// 原名：混排判据
    @Test func prescriptionNameNeedsSplitPredicate() {
    ...
    /// 原名：检验合体行三元组与参考范围
    @Test func labCombinedLineTripleAndReferenceRange() {
    ...
    /// 原名：检验定性与比较符结果原文保留不猜数
    @Test func qualitativeAndComparatorLabResultsKeepOriginalText() {
        let lab = ExtractionSpecRegistry.spec(for: "metric_sample")!
    ...
    /// 原名：票据合体费用行成行且合计进共享
    @Test func claimFeeLineYieldsRowAndTotalGoesToShared() {
    ...
    /// 原名：标签与值分列两个单元格时取下一格
    @Test func labelAndValueInSeparateCellsTakesNextCell() {
    ...
    /// 原名：标签无值与否认叙事不产字段
    @Test func emptyLabelAndNegatedNarrativeProduceNoFields() {
    ...
    /// 原名：续表页只得建议不借用
    @Test func continuationPageGetsHintOnlyWithoutBorrowing() {
