...
struct CardTemplateMatcherTests {
    ...
    /// 原名：检验页十项目一卡十行
    func labPageTenItemsOneCardTenRows() {
    ...
    /// 原名：就诊卡需类型判定
    func encounterCardNeedsKindDecision() {
    ...
    /// 原名：处方页每药一行
    func prescriptionPageOneRowPerMedication() {
    ...
    /// 原名：参考范围归行
    func referenceRangeAssignedToRow() {
    ...
    /// 原名：行级必填缺失标记
    func rowLevelRequiredMissingMarked() {
        let fields = [FieldDraft(key: "report_date", value: "2026-09-01", confidence: 0.6),
    ...
    /// 原名：无提取器卡类不匹配
    func noExtractorCardKindMismatch() {
    ...
    /// 原名：费用清单页每项一行且票据页空行
    func feeListPageOneRowPerItemAndInvoicePageEmptyRows() {
    ...
    /// 原名：处方新键归行与表头
    func prescriptionNewKeysAssignedToRowsAndHeader() {
        let fields = [FieldDraft(key: "drug_name", value: "阿莫西林胶囊", confidence: 0.9),
                      FieldDraft(key: "prescribed_at", value: "2026-09-01", confidence: 0.9),
                      FieldDraft(key: "hospital", value: "市一医院", confidence: 0.9),
    ...
    /// 原名：就诊叙事多段并入同键
    func encounterNarrativeMultiSegmentMergedSameKey() {
        let fields = [FieldDraft(key: "report_date", value: "2026-09-01", confidence: 0.6),
                      FieldDraft(key: "dept", value: "心内科", confidence: 0.6),
