...
let package = Package(
    ...
    dependencies: [
        ...
        // xctest-dynamic-overlay：swift-perception 的传递依赖（PerceptionCore → IssueReporting）。
        // @Perceptible 宏展开代码直接调用 IssueReporting 符号，而 Xcode 把 SwiftPM 静态库链入 App 时
        // 不会为「宏引入的传递依赖」补链接行（CI 34759901369 实证：Undefined symbols
        // IssueReporting.IssueReporter/FailureObserver/IssueContext）。与 GRDB 同纪律显式声明为产品依赖；
        // 版本区间与 swift-perception 2.0.12 的声明一致（from 1.13.0），由 Package.resolved 钉 1.13.1。
        ...
        .target(
            ...
            dependencies: [
                ...
                .product(name: "Perception", package: "swift-perception",
                         condition: .when(platforms: [.iOS, .macOS])),
                // IssueReporting：@Perceptible 宏展开的直接调用面（见上方依赖注记），与 Perception 同条件链接。
