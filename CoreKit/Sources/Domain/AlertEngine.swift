...
public enum WordingBlacklist {
    static let patterns: [(String, String)] = [
        ...
        ("可能是(.+?)症", "疾病名推断"),
        ("因为(.+?)所以", "因果句式"),
        // 审查修复（FR16.5 类别补全）：规范明确「处置建议（含停药/调药暗示）
        // 与因果表述」为否决类别——旧词表只有「因为…所以」单一因果句式，
        // 「建议停用该药/由于停药导致…」等模型生成展示文本命中
        // violation()==nil、BR-006 一票否决被静默绕过。
        // 处置词必须带引导动词（请/建议/应当/应该/考虑）——裸词会误伤
        // 应用自有固定模板（拒绝卡「为避免识别误差造成用药差错」、高危
        // 拒绝「调整或停用药物必须由医生决定」等安全话术本身含停药字样）
        ("(?:请|建议|应当|应该|考虑)(?:你|您)?(?:停用|停药|停止服用|调药|减量|加量|增大剂量|增加剂量|减少剂量|减小剂量)", "处置建议"),
        ("由于(.+?)(?:导致|引起|造成)", "因果表述"),
        ("建议服用", "治疗建议"),
        ("应该吃药", "治疗建议"),
        ("确诊", "诊断表述"),
