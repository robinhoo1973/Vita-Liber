...
struct AppRootView: View {
    ...
    /// 宽限锁定的墙钟死线（回前台竞态判定锚点，与任务取消解耦）
    @State private var graceDeadline: Date?
    /// 场景方向感知（区分「离开应用」与「返回应用」的 inactive）
    ...
    var body: some View {
        WithPerceptionTracking {
            ...
            .onChangeCompat(of: appState.lastBackupAt) { oldValue, newValue in
            ...
            .onChangeCompat(of: settingsStore.values[.authHealthRead]) { _, newValue in
