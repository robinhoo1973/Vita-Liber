...
#        只查语法；以下十二族类型错误仅 macOS L1 编译门禁可暴露，静态启发式左移拦截：
...
DEFAULT_REDLINE="$APP/App/Features/Onboarding/OnboardingViews.swift:$APP/App/Features/Reminders/RemindersViews.swift:$APP/App/Features/Observations/ObservationViews.swift:$APP/App/Features/Assistant/AssistantHistoryView.swift:$APP/App/Features/Emergency/EmergencyCareViews.swift:$APP/App/Features/Search/GlobalSearchView.swift:$APP/App/Features/Settings/HelpViews.swift:$APP/App/DesignSystem/SensitiveMediaContainer.swift:$APP/App/Features/Emergency/AlertViews.swift:$APP/App/Features/Medications/MedicationPlanViews.swift:$APP/App/Features/Medications/InventoryViews.swift:$APP/App/Features/Health/DeviceConnectionView.swift"
...
if ! command -v python3 >/dev/null 2>&1; then
  fail "无 python3 —— .strings 结构校验不可执行，不得空扫判 PASS（ERR#27）"
else
  STRINGS_SCAN="$(python3 - "$APP" <<'PYEOF'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
files = sorted(root.glob("Resources/Localization/*.lproj/Localizable.strings"))
...
for f in files:
    ...
    with open(f, encoding="utf-8") as fh:
        for lineno, raw in enumerate(fh, 1):
            ...
            if not m:
                bad.append(f"{f}:{lineno}: malformed: {line[:100]}")
                continue
            key = m.group(1)
            keys.append(key)
            if key in seen:
                bad.append(f"{f}:{lineno}: duplicate {key!r} (first {seen[key][0]}: {seen[key][1]!r})")
            else:
                seen[key] = (lineno, m.group(2))
    keysets[f] = set(keys)
print(f"__SCANNED__ {len(files)}")
if not files:
    print("FAIL: 0 个 .strings 文件被扫描")
    sys.exit(1)
if files:
    base = files[0]
    for f in files[1:]:
        missing = sorted(keysets[base] - keysets[f])
        extra = sorted(keysets[f] - keysets[base])
        if missing:
...
# ---------- [15] 类型层启发式门禁 ----------
section "15/17" "类型层启发式 —— 跨层 import 覆盖/Date·Double 混比/iOS 专用符号守卫/Linux 桩守卫外使用/any X? 拼写/nil→String 实参/长链高阶表达式/MainActor 跨隔离调用/Swift Charts 不存在符号/iOS 17 专用符号越界/非转义函数型参数赋存储属性/顶层类型嵌套成员误引（十二族 CI 实证左移）"
# 背景：App/（SwiftUI）在 Linux 无法编译，swiftc -parse 只查语法不查语义，
# 以下十二族类型错误只有 macOS L1 编译门禁才能暴露（每族均有 CI 实证）：
...
#   / nil 字面量传非可选 String 成员（34289498685）
#   / 单语句 ≥6 段高阶链式调用超 macOS 类型检查预算（34653854625）
#   / MainActor 跨隔离调用（34673049166）/ Swift Charts 不存在符号（34747651162）
...
#     HealthKitSyncService.SyncReport，'is not a member type' 仅 macOS L1 可见）。
...
if ! command -v python3 >/dev/null 2>&1; then
  ...
  if [ -z "$t_scanned" ]; then
    ...
    pass "$(printf '%s\n' "$THEUR" | sed -n 's/^__SCANNED__ //p') 个文件通过十二族启发式"
  fi
fi
